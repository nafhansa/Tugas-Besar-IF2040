...
# Paket_Langganan (not 150, realistic small set)
pakets = [
    ('Basic', '4.99', '720p', 1),
...
# Konten (300) -> first 150 are Film, next 150 are Serial_TV
konten_count = 300
konten = []
...
# Episode: create 2 episodes per serial -> 300 episodes
lines.append('-- Episode')
ep_id = 0
for i in range(150,300):
    title = konten[i][0].replace("'","''")
    # For each serial create 2 episodes in season 1
    for ep in range(1,3):
        ep_id += 1
...
# Audio (each konten 1-2 languages)
lines.append('-- Audio')
for c in konten:
...
# Subtitle (each konten 1-2 languages)
...
# Genre_Konten: assign 1-3 genres per konten
...
for i in range(150):
    ...
    num = num + str(i%100).zfill(2)  # ensure uniqueness and up to 18 chars
    num = num[:19]
    cards.append(num)
...
# Pelanggan (150) -> assign paket randomly, some with kartu
...
# Rating (150)
lines.append('-- Rating')
# ensure we create exactly 150 unique ratings (email_pelanggan, nama_profil, judul_konten)
r_set = set()
while len(r_set) < 150:
    prof = random.choice(profiles)
    c = random.choice(konten)[0]
